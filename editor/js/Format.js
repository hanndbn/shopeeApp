...
Format.prototype.refresh = function() {
  ...
  if (graph.isSelectionEmpty()) {
    ...
    } else if (this.getSelectionState().style.type === "SHARE_BUBBLE") {
      const socialPanel = div.cloneNode(false);
      this.panels.push(new CustomSocialFormatPanel(this, ui, socialPanel));
...
CustomFileFormatPanel.prototype.init = function() {
  ...
  const imageType = ss.style.imageType ? ss.style.imageType : "url";
  ...
  <div class="format-image-wrapper">
    ...
    <hr/>
    <div class="select-wrapper g-margin-top-10 mb-0 pr-2">
        <div class="input-box">
          <div class="w-100">Select image from</div>
  ...
  const { layout, clickHandle } = customLayout.customSelect(data, handleSelect);
  this.clickHandle = clickHandle;
  fileContainer.querySelector("#custom-select-wrapper").innerHTML = "";
  fileContainer.querySelector("#custom-select-wrapper").appendChild(layout);
  ...
  fileContainer.querySelector("#image-selection-container").innerHTML = "";
  fileContainer.querySelector("#image-selection-container").appendChild(layoutImageSelect);
...
CustomCardFormatPanel.prototype.init = function() {
  ...
  const cardCell = cells[0] ? cells[0] : null;
  let cardName = "";
  let labelCell = null;
  if (cardCell) {
    labelCell = cardCell.children.find(v => this.format.getCellInfo(v).style && this.format.getCellInfo(v).style.type === "CARD_LABEL");
    cardName = labelCell ? labelCell.value : "";
  }
        ...
        <div class="select-wrapper g-margin-top-10">
          <div class="input-box">
            <div class="font-size-wrapper">
                ...
                <input type="text" maxlength="10" class="w-50 mr-2" value="${cardName}" id="input-card-name"/>
            </div>
            </div>
        </div>
  ...
  //handle change card name
  $(cardContainer.querySelector("#input-card-name")).change(function(e) {
    if (labelCell) {
      graph.model.setValue(labelCell, e.target.value);
    }
  });
    ...
    <div class="format-link-wrapper row no-gutters">
        ...
        <div class="select-wrapper g-margin-top-10">
          ...
          <div class="input-box">
            ...
            <div id="custom-select-wrapper" class="ml-2 flex-fill"></div>
          </div>
        </div>
    </div>
  </div>
...
CustomSocialFormatPanel = function(format, editorUi, container) {
  CustomBaseFormatPanel.call(this, format, editorUi, container);
  this.init();
};

CustomSocialFormatPanel.prototype.init = function() {
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
  var ss = this.format.getSelectionState();
  ...
  <div class="format-image-wrapper">
    <div class="format-image-header">Social Editor</div>  
    <hr/>
    <div class="format-link-wrapper row no-gutters">
        <div class="form-check w-100 d-flex mb-2">
            <div class="custom-select-box mr-2 select-option" data-social="facebook">
                <span class="${shareFacebook ? "checkmark" : ""}"/>
            </div>
            <label class="form-check-label" for="format-link-open-in-modal">Share in Facebook</label>
        </div>
        <div class="form-check w-100 d-flex mb-2">
            <div class="custom-select-box mr-2 select-option"  data-social="twitter">
                <span class="${shareTwitter ? "checkmark" : ""}"/>
            </div>
  ...
  `;
  $(socialContainer.querySelectorAll(".select-option")).click(function(e) {
    const type = e.currentTarget.dataset.social;
    if (type) {
      let shareSocialListTmp = [...shareSocialList];
      if (shareSocialListTmp.includes(type)) {
        shareSocialListTmp = shareSocialListTmp.filter(v => v !== type);
      } else {
        shareSocialListTmp.push(type);
      }
      customUtils.setCellStyles(graph, { shareSocial: shareSocialListTmp.join(",") });
    }
  });
...
CustomPDMFormatPanel.prototype.init = function() {
  ...
  let pdmInfo = ss.style.pdmInfo ? JSON.parse(decodeURIComponent(ss.style.pdmInfo)) : [];
  ...
  <div class="format-image-wrapper">
    ...
    <hr/>
        <div class="select-wrapper g-margin-top-10 mb-3 pr-2">
          <div class="input-box">
            <div class="w-100">Front Page Background</div>
            <div id="custom-select-wrapper" class="flex-fill"></div>
          </div>
          <div class="input-box" id="image-selection-container">
          </div>
  ...
  pdmInfo.map(v => {
    ...
    v.packs && v.packs.map(k => {
      const packImage = k.packImage ? k.packImage : "/content/images/editor/img/defaultPicture.jpg";
      ...
      packContent.innerHTML = `
          <div class="col-8 pl-2">
                    <div class="input-box">
                      <div class="font-size-wrapper pdm-input-wrapper">
                          <div class="w-100 d-flex flex-wrap mb-2 justify-content-center">
                            <div class="position-relative image-preview-wrapper">
                               <img src="${packImage}" class="image-preview">
                               <div class="image-type-menu input-select-content" style="display: none">
                                    <div class="input-select-item input-select-image" data-row="${v.rowId}" data-pack="${k.packId}" data-type="url">Url</div>     
                                    <div class="input-select-item input-select-image" data-row="${v.rowId}" data-pack="${k.packId}" data-type="library">Library</div>
                               </div>
                            </div>
                          </div>
                      </div>
          ...
          <div class="col-4 d-flex justify-content-center align-items-center">
            <a class="far fa-edit mr-2" target="_blank" href="?app=${editor.getFilename()}&cardId=${graph.getSelectionCells()[0].getId()}&packId=${k.packId}"></a>
            <span class="far fa-trash-alt delete-pack" data-row="${v.rowId}" data-pack="${k.packId}"></span>
          </div>
      `;
      rowContent.querySelector(`#pack-detail-content-${v.rowId}`).appendChild(packContent);
    });

    pdmContainer.querySelector("#row-content").appendChild(rowContent);
  });
  ...
  $(pdmContainer.querySelector("#add-row")).click(function(e) {
    pdmInfo.push({
      ...
      packs: [
        {
          packId: uuidv1(),
          packImage: "/content/images/editor/img/defaultPicture.jpg",
          packImageSelected: [],
          packImageUrl: "",
          packName: "pack 1"
        }
    ...
    customUtils.setCellStyles(graph, { pdmInfo: encodeURIComponent(JSON.stringify(pdmInfo)) });
  });
  ...
  $(pdmContainer.querySelectorAll(".add-pack")).click(function(e) {
    ...
    if (rowIdx > -1) {
      ...
      row.packs.push({
        ...
        packImageSelected: [],
        packImageUrl: "",
        packName: `pack ${row.packs.length + 1}`
      });
    }
    customUtils.setCellStyles(graph, { pdmInfo: encodeURIComponent(JSON.stringify(pdmInfo)) });
  });
  ...
  const { layout, clickHandle } = customLayout.customSelect(data, handleSelect);
  this.clickHandle = clickHandle;
  pdmContainer.querySelector("#custom-select-wrapper").innerHTML = "";
  pdmContainer.querySelector("#custom-select-wrapper").appendChild(layout);
  ...
  const { layoutImageSelect } = customLayout.customImageSelect(imageData);
  pdmContainer.querySelector("#image-selection-container").innerHTML = "";
  pdmContainer.querySelector("#image-selection-container").appendChild(layoutImageSelect);


  // change icon pack
  $(pdmContainer.querySelectorAll(".image-preview-wrapper")).click((e) => {
    $(e.currentTarget.querySelector(".input-select-content")).toggle();
  });

  $(pdmContainer.querySelectorAll(".input-select-image")).click((e) => {
    ...
    if (type === "url") {
      ...
      if (rowIdx > -1) {
        ...
        const packIdx = row.packs.findIndex(k => k.packId === packId);
        if (packIdx > -1) {
          pack = row.packs[packIdx];
        }
      }
      const fileSelected = pack && pack.packImageSelected ? pack.packImageSelected : [];
      const displayData = {
        fileType: "image",
...
CustomDiagramFormatPanel.prototype.init = function() {
  ...
  if (graph.isEnabled()) {
    ...
    Object.keys(cells).map((k) => {
      const cell = cells[k];
      const labelCell = cell.children ? cell.children.find(v => this.format.getCellInfo(v).style && this.format.getCellInfo(v).style.type === "CARD_LABEL") : null;
      const cardName = labelCell ? labelCell.value : "";
      ...
      if (cellInfo.style.type === "CARD") {
        ...
        <div class="row no-gutters layer-card-wrapper">
            ...
            <div class="col-6 layer-card-id text-center">${cardName}</div>
