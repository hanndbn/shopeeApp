...
Format.prototype.refresh = function() {
  ...
  if (graph.isSelectionEmpty()) {
    ...
    if (ss.style.type === "IMAGE") {
      var imagePanel = div.cloneNode(false);
      // stylePanel.style.display = 'none';
      this.panels.push(new ImageFormatPanel(this, ui, imagePanel));
...
ImageFormatPanel = function(format, editorUi, container) {
  this.format = format;
  this.editorUi = editorUi;
  this.container = container;
  this.init();
...
ImageFormatPanel.prototype.init = function() {
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
  var ss = this.format.getSelectionState();
  ...
  <div class="format-image-wrapper">
    <div class="format-image-header">Media Editor</div>  
    <hr/>
    <div class="format-image-subheader">Select a media or upload a media URL</div>
    <div class="row no-gutters format-image-content-wrapper" id="format-image-content-wrapper">
       
    </div>
  </div>
  `;
  filesListing["image"] && filesListing["image"].forEach(function(image) {
    const divImage = document.createElement("div");
    divImage.className = "col-4 format-image-content";
    divImage.innerHTML = `
          <div>
                <img class="image-format-img" src="${image.url}"/>
                <span class="format-image-close" data-name="${image.fileName}">x</span>
          </div>
    `;
    $(imageContainer.querySelector("#format-image-content-wrapper")).append(divImage);
  });
  ...
  divImportImage.innerHTML = `
    ...
    `;
  $(imageContainer.querySelector("#format-image-content-wrapper")).append(divImportImage);


  $(imageContainer.querySelector("#upload-image")).click(function() {
    $("#myModal").html(imageUpload());
    ...
    $("#uploadFile").click(function(e) {
      ...
      requestUploadFile(userid, formData, function(data) {
        requestFilesWithType(userid, "image");
...
ImageFormatPanel.prototype.destroy = function() {
};
