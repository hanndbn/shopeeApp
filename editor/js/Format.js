...
Format.prototype.refresh = function() {
  ...
  if (graph.isSelectionEmpty()) {
    ...
    if (this.getSelectionState().style.type === "IMAGE") {
      var imagePanel = div.cloneNode(false);
      // stylePanel.style.display = 'none';
      this.panels.push(new FileFormatPanel(this, ui, imagePanel, "image", "image/*"));
      ...
      // stylePanel.style.display = 'none';
      this.panels.push(new FileFormatPanel(this, ui, filePanel, "pdf", "application/pdf"));
      ...
      // stylePanel.style.display = 'none';
      this.panels.push(new LinkFormatPanel(this, ui, linkPanel));
...
FileFormatPanel = function(format, editorUi, container, fileType = "", fileTypeAccept = "*") {
  BaseFormatPanel.call(this, format, editorUi, container);
...
FileFormatPanel.prototype.init = function() {
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
...
FileFormatPanel.prototype.destroy = function() {
};

CustomTextFormatPanel = function(format, editorUi, container) {
  BaseFormatPanel.call(this, format, editorUi, container);
  this.init();
};

CustomTextFormatPanel.prototype.init = function() {
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
  var ss = this.format.getSelectionState();
  ...
  const fontFamily = ss.style.fontFamily ? ss.style.fontFamily : "";
  const fontSize = ss.style.fontSize ? ss.style.fontSize : "12";
  // bold italic underline
  const fontStyle = ss.style.fontSize ? ss.style.fontStyle : "";
  const fontColor = ss.style.fontColor ? ss.style.fontColor : "";
  ...
  <div class="format-image-wrapper">
    ...
    <div class="format-link-wrapper row no-gutters">
        ...
        <div class="color-wrapper g-margin-left-10">
            <div class="format-color-preview" style="background-color: white"></div>
            <span class="fa fa-pencil-alt format-color-edit" id="format-color-picker"/>
        </div>
    </div>
  </div>
  `;
  // $(textContainer.querySelector('.mdb-select')).materialSelect();
...
LinkFormatPanel = function(format, editorUi, container) {
  BaseFormatPanel.call(this, format, editorUi, container);
  this.init();
};

LinkFormatPanel.prototype.init = function() {
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
  var ss = this.format.getSelectionState();
  ...
  <div class="format-image-wrapper">
    <div class="format-image-header">Link Editor</div>  
    <hr/>
    <div class="format-image-subheader">Input a valid URL</div>
    </div>
    <div class="format-link-wrapper row no-gutters">
...
LinkFormatPanel.prototype.destroy = function() {
};
